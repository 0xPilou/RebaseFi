...
describe("TimeOptimizer Unit Tests", function () {
  ...
  it("should reinvest 50% of the rebase in Curve Beefy Vault", async () => {
    ...
    await timeOptimizer.connect(user).reinvestInMooCurve(mooCurveZap.address, basisPoint);
  ...
  it("should prevent non-owner to interact with the contract", async () => {
    ...
    // Assertion : Transaction should revert as the caller is not the owner of the contract
    await truffleAssert.reverts(timeOptimizer.connect(nonOwner).reinvestInMooCurve(mooCurveZap.address, 5000));
